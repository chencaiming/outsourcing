...
uses
  Windows,
  Classes, SmartListView, ComCtrls, Forms;
...
procedure TThreadsPool.KillAllThreads;
var
    i: Integer;
begin
    // Check for dead threads
    ...
    // Dead thread moverd to other list.
    // New thread created to replace dead one
            TProcessorThread(FThreads[i]).Suspend;
            TProcessorThread(FThreads[i]).Terminate;
            FThreadsKilling.Add(FThreads[i]);
            try
                FThreads[i] := TProcessorThread.Create(Self);
                ...
                begin
                    FThreads[i] := nil;
{$IFNDEF NOLOGS}
                    WriteLog('TProcessorThread.Create raise: ' + e.ClassName + #13#10#9'Message: ' + e.Message, 9);
{$ENDIF}
                end;
            end;
        end;
