...
uses
    SysUtils, Classes, Windows, RegExpr,  IdIOHandler, IdIOHandlerSocket, IdIOHandlerStack,
  IdSSL, IdSSLOpenSSL, IdBaseComponent, IdComponent, IdTCPConnection,
  IdTCPClient, IdHTTP,ExtCtrls, ComCtrls;
...
//ÍøÂçÍ¨Ñ¶
function HttpPost(AURL: string; PostData: string;Referer: string='';ContentType: string=''): string;

//listViewÅÅÐò
function CustomSortProc(Item1, Item2: TListItem; ParamSort: Integer): Integer;stdcall;

procedure lvCompare(nmm_Tag: Integer; lv: TListView; Sender: TObject; Item1,Item2: TListItem; Data: Integer;var Compare: Integer);
procedure lvColumnClick(nmm_intType: Integer; lv: TListView; Sender: TObject; Column:TListColumn);
//Á÷²Ù×÷==================
//...
...
//...
function  CMemoryStreamToStr(_MemoryStream: TMemoryStream): string;
...
procedure OpenUrlInIE(Url: string);

var
    GUpcaseTable: array[0..255] of char;
    GUpcaseLUT: Pointer;
    I: Integer;
    nmg_Tag, ColumnToSort: Integer;
    m_bSort: boolean;
...
function GetIEAppPath:string;
    function  regRead(_rootKey:HKEY;_openKey,_Name:string):string;
    var
        regTemp :TRegistry;
    begin
        Result:='';
        ...
        try
            RootKey   := _rootKey;
            if OpenKey(_openKey, false) then
...
begin
    Result := regRead(HKEY_LOCAL_MACHINE,'SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\IEXPLORE.EXE','');
    if Result='' then Result := 'C:\Program Files\Internet Explorer\IEXPLORE.EXE';
...
begin
    ...
    if IEPath='' then
    begin
//        ShowMessage_OK('ÎÞ·¨»ñÈ¡IEÂ·¾¶£¬ÇëÊÖ¶¯´ò¿ªÍøÖ·:'+Url);
        Exit;
    end;
    ShellExecute(0,nil,PChar(IEPath),PChar(Url),nil,SW_SHOWNORMAL);
end;
...
procedure lvColumnClick(nmm_intType: Integer; lv: TListView; Sender: TObject; Column:TListColumn);
const nmm_Str: array[1..2] of string = ('A','D');
begin
    if lv.Tag <=0 then
    begin
        if ColumnToSort <> -1 then
        begin
            Column.Caption := lv.Columns[Column.index].Caption + nmm_Str[2];
        end
        else
        begin
            Column.Caption := Column.Caption + nmm_Str[2];
        end;
        lv.tag := 1;

    end
    else if lv.tag = 1 then
    begin
        if ColumnToSort <> -1 then
        begin
            lv.columns[ColumnToSort].Caption := Copy(lv.Columns[ColumntoSort].Caption,1,Length(lv.Columns[ColumnToSort].Caption) - 2);
            Column.Caption := Column.Caption + nmm_Str[1];
        end
        else begin
            Column.Caption := Copy(Column.Caption, 1, Length(Column.Caption)-2)+nmm_Str[1];
        end;
        lv.Tag := 2;
    end
    else if lv.Tag = 2 then
    begin
        if ColumnToSort <> -1 then
        begin
            lv.Columns[ColumnToSort].Caption := copy(lv.Columns[ColumnToSort].Caption, 1,Length(lv.Columns[ColumnToSort].Caption)-2);
        ...
        begin
            Column.Caption := copy(Column.Caption, 1, Length(Column.Caption) - 2) + nmm_Str[2];
        end;
        lv.Tag := 1;
...
begin
    ...
    begin
        if lv.Tag = 0 then
        begin
            Compare := Integer(StrToFloat(Item1.SubItems[ColumnToSort -1]) > StrToFloat(Item2.SubItems[columntoSort-1]));
        ...
        begin
            ...
            begin
                Compare := Integer(StrToFloat(Item2.Caption)>StrToFloat(Item1.Caption));
            end
            else
            Compare := Integer(StrToFloat(Item2.SubItems[columnToSort-1])>StrToFloat(Item1.SubItems[columnToSort-1]));
        end
        else if lv.Tag = 2 then
        begin
            Compare := Integer(StrToFloat(Item1.SubItems[ColumnToSort-1]) > StrToFloat(Item2.SubItems[ColumnToSort-1]));
...
begin
    ...
    try
        ...
        RequestStream.WriteString(PostData);
        IdHttp.Request.UserAgent := 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/53.0.2785.116 Safari/537.36';
        IdHttp.Request.Referer := Referer;
        IdHttp.Request.ContentType := ContentType;//'application/x-www-form-urlencoded; charset=UTF-8';
        IdHttp.Post(AURL, RequestStream, ResponseStream);
...
function GetMatchs(strText: string; strExp: string; Matchs: TStrings = nil; bMS: Boolean = true; bMG: Boolean = false; bMR: Boolean = false; Ver: byte = 1; iContent: integer = 0): integer;
    ...
    //Acai Èç¹ûÊ¹ÓÃwidestring, ÖÐÓ¢ÎÄ¼ÆËã»á´íÎó...ÔÝÊ±ÎÞ½â
    ...
    begin
        ...
        //2014-11-24 17:14 Acai Êä³öËùÔÚÐÐ
