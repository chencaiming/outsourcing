...
begin
  ...
  begin
    ...
    else (* Down south *) if (beforeStandardDate or afterDaylightDate) then
        Result := TIME_ZONE_ID_DAYLIGHT;
  end
  else
    (* No transition date *)
...
var
  ...
  function get(var v: Word; c: SOChar): Boolean;
  {$IFDEF HAVE_INLINE}inline;
  {$ENDIF}
  begin
...
type
  ...
  function ishex(const c: Char): Boolean;
  {$IFDEF HAVE_INLINE}inline;
  {$ENDIF}
  begin
